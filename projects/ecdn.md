...
3. 图形数据查询失败，根据日志发现SQL查询结果返回了错误，报错内容为查询语句长度超限，ClickHouse默认参数max_query_size为2.5MB，具体原因是聚合一段时间内一批服务器的数据，通过wherein id的方式查询，但是id过多导致查询语句超长，改为添加其他维度主键和并发分批查询的方式解决。

4. Prometheus指标查询拼接主机名过长返回失败或空，在查询数量超过100的情况下直接全量查询，解决了速度慢合查询失败的问题。